...
class KambrikArgBuilder<SRC, A : ArgumentBuilder<SRC, *>>(val arg: A) :
    ...
    ) = argument(IdentifierArgumentType.identifier(), word, items, func)

    fun argInt(
        word: String, range: IntRange? = null,
        items: SuggestionProvider<SRC>? = null, func: ArgDslTyped<SRC, Int> = {}
    ) = argument(if (range != null) IntegerArgumentType.integer(
        range.first,
        range.last
    ...
    ) = argument(NumberRangeArgumentType.intRange(), word, items, func)
